...
# Inicializar norm como None antes de la lectura de datos
norm = None
...
for i, (label, base_path) in enumerate(base_paths.items()):
    ...
    for temp in temps:
        ...
        if os.path.exists(hb_file):
            ...
            store_data = data[-data.size // 10:] / norm if norm != 0 else data[-data.size // 10:]  # Evitar división por cero
    ...
    ax.errorbar(temps, ave_data, yerr=std_data, 
    ...
    temp_at_05 = find_temp_for_fraction(temps, ave_data, target_fraction=0.5)
...
ax2.set_xlabel("Longitud del sticky end", fontsize=10)
