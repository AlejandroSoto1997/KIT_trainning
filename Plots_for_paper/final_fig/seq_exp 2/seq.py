...
fig, axs = plt.subplots(2, 1, figsize=(7.3, 13))

df1 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[11]))
axs[2].plot(df1["Temperature(C)"], df1["Fraction of bases unpaired at equilibrium"], color = '#9e0142', label = df_tm_info_1['molecule_clean'].iloc[0])
axs[2].legend(loc='best', fontsize=10)


df2 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[12]))
axs[2].plot(df2["Temperature(C)"], df2["Fraction of bases unpaired at equilibrium"], color = '#d53e4f', label = df_tm_info_1['molecule_clean'].iloc[1])
axs[2].legend(loc='best', fontsize=10)


df3 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[10]))
axs[2].plot(df3["Temperature(C)"], df3["Fraction of bases unpaired at equilibrium"], color = '#f46d43', label = df_tm_info_1['molecule_clean'].iloc[2])
axs[2].legend(loc='best', fontsize=10)

df4 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[9]))
axs[2].plot(df4["Temperature(C)"], df4["Fraction of bases unpaired at equilibrium"], color = '#fdae61', label = df_tm_info_1['molecule_clean'].iloc[3])
axs[2].legend(loc='best', fontsize=10)

df5 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[6]))
axs[2].plot(df5["Temperature(C)"], df5["Fraction of bases unpaired at equilibrium"], color = '#fee08b', label = df_tm_info_1['molecule_clean'].iloc[4])
axs[2].legend(loc='best', fontsize=10)

df6 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[5]))
axs[2].plot(df6["Temperature(C)"], df6["Fraction of bases unpaired at equilibrium"], color = '#ffffbf', label = df_tm_info_1['molecule_clean'].iloc[5])
axs[2].legend(loc='best', fontsize=10)

df7 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[7]))
axs[2].plot(df7["Temperature(C)"], df7["Fraction of bases unpaired at equilibrium"], color = '#e6f598', label = df_tm_info_1['molecule_clean'].iloc[6])
axs[2].legend(loc='best', fontsize=10)

df8 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[8]))
axs[2].plot(df8["Temperature(C)"], df8["Fraction of bases unpaired at equilibrium"], color = '#abdda4', label = df_tm_info_1['molecule_clean'].iloc[7])
axs[2].legend(loc='best', fontsize=10)

df9 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[1]))
axs[2].plot(df9["Temperature(C)"], df9["Fraction of bases unpaired at equilibrium"], color = '#66c2a5', label = df_tm_info_1['molecule_clean'].iloc[8])
axs[2].legend(loc='best', fontsize=10)

df10 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[0]))
axs[2].plot(df10["Temperature(C)"], df10["Fraction of bases unpaired at equilibrium"], color = '#3288bd', label = df_tm_info_1['molecule_clean'].iloc[9])
axs[2].legend(loc='best', fontsize=10)

df11 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[3]))
axs[2].plot(df11["Temperature(C)"], df11["Fraction of bases unpaired at equilibrium"], color = '#9673b9', label = df_tm_info_1['molecule_clean'].iloc[10])
axs[2].legend(loc='best', fontsize=10)

df12 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[4]))
axs[2].plot(df12["Temperature(C)"], df12["Fraction of bases unpaired at equilibrium"], color = '#5e4fa2', label = df_tm_info_1['molecule_clean'].iloc[11])
axs[2].legend(loc='best', fontsize=10)

df13 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[2]))
axs[2].plot(df13["Temperature(C)"], df13["Fraction of bases unpaired at equilibrium"], color = '#2c1e50', label = df_tm_info_1['molecule_clean'].iloc[12])
axs[2].legend(loc='best', fontsize=10)



# Configuración del zoom
axins = axs[2].inset_axes([0.7, 0.1, 0.25, 0.25])  # (x0, y0, width, height) del panel secundario
...
axs[2].tick_params(axis='both', which='major', labelsize=25)  # Aquí ajusta el tamaño de fuente según tus preferencias
...
axs[2].set_xlabel('Temperature ($^{o}$C)', fontsize=25)
axs[2].set_ylabel('Fraction unbounded', fontsize=25)
...
axs[3].scatter(df_tm_info_1['nucleotides'], df_tm_info_1['Tm'], marker='o', color=colors, s=150, edgecolor='black')
...
for i, txt in enumerate(df_tm_info_1['Tm']):
    ...
    axs[3].text(x + 0.03, y + 0.08, f'{txt:.1f}', fontsize=12, ha='center', va='bottom')
    
axs[3].scatter(df_tm_info_1['nucleotides'], df_tm_info_1['Tm'], marker='o', color=colors, s=150, edgecolor='black')
#axs[3].set_title('Tm vs Concentración')
axs[3].set_xlabel('Length sticky end (no. nucleotides)',fontsize=25)
axs[3].set_ylabel('Melting temperature ($^{o}$C)',fontsize=25)
axs[3].set_yticks(np.arange(62, 68, 1))
axs[3].set_xticks(np.arange(0, 16, 4))  # Especificar los ticks del eje y
axs[3].tick_params(axis='both', which='major', labelsize=25)
#axs[3].set_ylim(66.5, 68)
...
# Repetir este patrón para cada DataFrame de las curvas de fusión de ADN de NUPACK restantes
