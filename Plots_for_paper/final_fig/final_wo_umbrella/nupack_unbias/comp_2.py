...
axs[1,0].plot(df1["Temperature(C)"], 1-df1["Fraction of bases unpaired at equilibrium"], color = colors_1[0], label = df_tm_info_1['molecule_clean'].iloc[4])

#df2 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[14]))
#axs[1,0].plot(df2["Temperature(C)"], 1-df2["Fraction of bases unpaired at equilibrium"], color = colors_1[1], label = df_tm_info_1['molecule_clean'].iloc[6])

#df3 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[3]))
#axs[1,0].plot(df3["Temperature(C)"], 1-df3["Fraction of bases unpaired at equilibrium"], color = colors_1[2], label = df_tm_info_1['molecule_clean'].iloc[16])

df4 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[0]))
axs[1,0].plot(df4["Temperature(C)"], 1-df4["Fraction of bases unpaired at equilibrium"], color = colors_1[3], label = df_tm_info_1['molecule_clean'].iloc[0])

df5 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[13]))
axs[1,0].plot(df5["Temperature(C)"], 1-df5["Fraction of bases unpaired at equilibrium"], color = colors_1[4], label = df_tm_info_1['molecule_clean'].iloc[14])

df6 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[16]))
axs[1,0].plot(df6["Temperature(C)"], 1-df5["Fraction of bases unpaired at equilibrium"], color = colors_1[5], label = df_tm_info_1['molecule_clean'].iloc[15])

df7 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[12]))
axs[1,0].plot(df7["Temperature(C)"], 1-df7["Fraction of bases unpaired at equilibrium"], color = colors_1[6], label = df_tm_info_1['molecule_clean'].iloc[13])

df8 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[11]))
axs[1,0].plot(df8["Temperature(C)"], 1-df8["Fraction of bases unpaired at equilibrium"], color = colors_1[7], label = df_tm_info_1['molecule_clean'].iloc[12])

df9 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[8]))
axs[1,0].plot(df9["Temperature(C)"], 1-df9["Fraction of bases unpaired at equilibrium"], color = colors_1[8], label = df_tm_info_1['molecule_clean'].iloc[9])

df10 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[7]))
axs[1,0].plot(df10["Temperature(C)"], 1-df10["Fraction of bases unpaired at equilibrium"], color = colors_1[9], label = df_tm_info_1['molecule_clean'].iloc[8])

df11 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[9]))
axs[1,0].plot(df11["Temperature(C)"], 1-df11["Fraction of bases unpaired at equilibrium"], color = colors_1[10], label = df_tm_info_1['molecule_clean'].iloc[10])

df12 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[10]))
axs[1,0].plot(df12["Temperature(C)"], 1-df12["Fraction of bases unpaired at equilibrium"], color = colors_1[11], label = df_tm_info_1['molecule_clean'].iloc[11])

df13 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[2]))
axs[1,0].plot(df13["Temperature(C)"], 1-df13["Fraction of bases unpaired at equilibrium"], color = colors_1[12], label = df_tm_info_1['molecule_clean'].iloc[2])

df14 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[1]))
axs[1,0].plot(df14["Temperature(C)"], 1-df14["Fraction of bases unpaired at equilibrium"], color = colors_1[13], label = df_tm_info_1['molecule_clean'].iloc[1])

df15 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[5]))
axs[1,0].plot(df15["Temperature(C)"], 1-df15["Fraction of bases unpaired at equilibrium"], color = colors_1[14], label = df_tm_info_1['molecule_clean'].iloc[5])

df16 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[6]))
axs[1,0].plot(df16["Temperature(C)"], 1-df16["Fraction of bases unpaired at equilibrium"], color = colors_1[15], label = df_tm_info_1['molecule_clean'].iloc[7])

df17 = pd.read_csv(os.path.join(directory_bases_profiles_1, archivos_bases_profiles_1[4]))
axs[1,0].plot(df17["Temperature(C)"], 1-df17["Fraction of bases unpaired at equilibrium"], color = colors_1[16], label = df_tm_info_1['molecule_clean'].iloc[3])
# Añadir leyenda con dos columnas
axs[1,0].legend(loc='lower left', fontsize=10, ncol=2)

# Configuración del zoom
axins = axs[1,0].inset_axes([0.7, 0.58, 0.25, 0.25])  # (x0, y0, width, height) del panel secundario
...
axs[1,0].set_ylabel("Fraction unbounded", fontsize=10)
axins.grid(True)
...
axs[1,0].tick_params(axis='both', which='major', labelsize=10)  # Aquí ajusta el tamaño de fuente según tus preferencias
...
axs[1,0].set_xlabel('Temperature ($^{o}$C)', fontsize=12)
axs[1,0].axhline(y=0.5, color='black', alpha=0.4, linestyle='--', linewidth=1, xmin=0.47, xmax=1)
#axs[1,0].tick_params(axis='y', which='both', left=True, right=True, labelleft=False, labelright=False)

#axs[1,0].set_ylabel('Fraction unbounded', fontsize=12)
axs[1,0].text(80, 0.97, '100 mM NaCl',  
               fontsize=10, color='black',  
               ha='left', va='top', 
               bbox=dict(boxstyle='square,pad=0.1', facecolor='white', edgecolor='none', alpha=0.1))

# Anotación en la segunda línea
axs[1,0].text(80, 0.93, '1 $\mu$M',  
               fontsize=10, color='black',  
               ha='left', va='top',
               bbox=dict(boxstyle='square,pad=0.1', facecolor='white', edgecolor='none', alpha=0.1))
...
axs[1, 1].set_xticks(np.arange(0, 17, 1))  # Especificar los ticks del eje x
...
# Ajustar el tamaño y la posición de la barra de colores
cbar_ax = fig.add_axes([0.59, 0.12, 0.38, 0.03])  # [left, bottom, width, height]
