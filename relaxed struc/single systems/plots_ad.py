...
for subfolder in subfolders:
    folder_path = os.path.join(base_directory, subfolder, 'y_shape', 'duplex+se')
    ...
    if os.path.exists(file_path):
        ...
        # Calcula y almacena la mediana de la tercera columna
        median_energy = df['Energy3'].median()
        median_energy_dict[subfolder] = median_energy
        print(f'Median Total Energy for {subfolder}: {median_energy}')
    else:
        print(f'File not found: {file_path}')

# Crea un DataFrame a partir del diccionario de medianas
median_energy_df = pd.DataFrame(list(median_energy_dict.items()), columns=['Subfolder', 'MedianEnergy'])

# Separa la columna "Subfolder" en tres columnas usando el guion bajo como separador
median_energy_df[['Salt', 'salt_conc', 'temp']] = median_energy_df['Subfolder'].str.split('_', expand=True)

# Elimina la letra "C" y convierte la columna "temp" a número
median_energy_df['temp'] = median_energy_df['temp'].str.rstrip('C').astype(float)
# Convierte la columna "salt_conc" a número
median_energy_df['salt_conc'] = median_energy_df['salt_conc'].astype(float)

# Ordena el DataFrame por temperatura y concentración de sal
median_energy_df.sort_values(['temp', 'salt_conc'], inplace=True)

# Crea dos gráficos subplots
fig, axes = plt.subplots(1, 2, figsize=(15, 6))

# Gráfico 1: Salt Concentration vs Temperature
for salt_conc, group_df in median_energy_df.groupby('salt_conc'):
    axes[0].plot(group_df['temp'], group_df['MedianEnergy'], marker='o', label=f'Salt conc. = {salt_conc} mM')
...
for temp, group_df in median_energy_df.groupby('temp'):
    axes[1].plot(group_df['salt_conc'], group_df['MedianEnergy'], marker='o', label=f'Temp. = {temp}°C')
